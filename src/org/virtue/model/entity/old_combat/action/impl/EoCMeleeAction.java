 ...
 * @since 23/01/2015
 */
public class EoCMeleeAction extends CombatAction {
	...
	public void attack(Entity attacker, Entity defender, boolean successful) {
		if (successful) {
			...
			double hitMissProbability = Math.random();//att / def;
			
			if ((0.05 < hitMissProbability) && (hitMissProbability < 0.90))
				successHit = true;
			else 
				successHit = false;
			if (hitMissProbability < Math.random())
				successHit = false;
			if (successHit) {
...
//						defender.getCombat().queueBars(Bar.HITPOINTS);
				...
				if ((defender.getCombat().getMaxHitpoints() == damage) && (defender.getCombat().getHitpoints() == defender.getCombat().getMaxHitpoints())) {
					...
					if (damage > 200)
						defender.getCombat().queueHits(new Hit(damage, HitType.MELEE_DAMAGE_2));
					else if (damage < 200)
