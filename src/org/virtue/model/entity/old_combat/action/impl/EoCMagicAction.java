 ...
 * @since 23/01/2015
 */
public class EoCMagicAction extends CombatAction {
	...
	public void attack(Entity attacker, Entity defender, boolean successful) {
		if (successful) {
			...
			if (defender instanceof Player) {
				...
				Item magic = player.getInvs().getContainer(ContainerState.EQUIPMENT).get(CombatVariables.SLOT_WEAPON);
				Random rn = new Random();
				int mageAccuracy = rn.nextInt(magic.getType().getMagicAccuracy());
				int levelAccBon = rn.nextInt((attacker.getCombat().getLevelAccuracyBonus(player.getSkills().getBaseLevel(SkillType.MAGIC))));
				...
				double weaponBonus = mageAccuracy * CombatFormulas.getMeleeWeaponSpeed(magic.getType());
				if (getPercentage(hitChance))
					damage = (int) (Math.floor(weaponBonus * 2.5));
...
//					defender.getCombat().queueBars(Bar.HITPOINTS);
				...
				player.getSkills().addExperience(SkillType.MAGIC, combatXp);
				player.getSkills().addExperience(SkillType.CONSTITUTION, hpXp);
				
				if (magic.getType().name.contains("Polypore staff")) {
					if (magic != null && attacker instanceof Player) {
						((Player) attacker).getDispatcher().sendEvent(SceneUpdateEventEncoder.class, new SceneUpdateEventContext(new Projectile(attacker.getCurrentTile(), defender.getCurrentTile(), defender, getMagicProjectileGFX(magic), 17, 30, 10, 31, 32)));
					}
					if (magic != null && defender instanceof Player) {
						((Player) defender).getDispatcher().sendEvent(SceneUpdateEventEncoder.class, new SceneUpdateEventContext(new Projectile(attacker.getCurrentTile(), defender.getCurrentTile(), defender, getMagicProjectileGFX(magic), 17, 30, 10, 31, 32)));
					}
				}
				...
				int mageAccuracy = rn.nextInt(magic.getType().getMagicAccuracy());
				int levelAccBon = rn.nextInt((attacker.getCombat().getLevelAccuracyBonus(player.getSkills().getBaseLevel(SkillType.MAGIC))));
				damage = (int) (Math.floor(levelAccBon));
			...
			if ((defender.getCombat().getMaxHitpoints() == damage) && (defender.getCombat().getHitpoints() == defender.getCombat().getMaxHitpoints())) {
				defender.getCombat().queueHits(new Hit(damage, HitType.MAGIC_DAMAGE)); //This should only be instant kill with deathtouch darts
			} else {
				if (damage > 200)
					defender.getCombat().queueHits(new Hit(damage, HitType.MAGIC_DAMAGE_2));
				else if (damage < 200)
...
//		defender.getCombat().queueBars(Bar.HITPOINTS);
	...
	public int getMagicProjectileGFX(Item weapon) {
