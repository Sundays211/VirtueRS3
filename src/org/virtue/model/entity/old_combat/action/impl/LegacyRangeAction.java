...
public class LegacyRangeAction extends CombatAction {
	...
	public void attack(Entity attacker, Entity defender, boolean successful) {
		if (successful) {
			...
			if (weapon.getType().name.contains("Crystal bow") || weapon.getType().name.contains("Zaryte bow") ||
				...
				if (weapon != null && defender instanceof Player) {
					((Player) defender).getDispatcher().sendEvent(SceneUpdateEventEncoder.class, new SceneUpdateEventContext(new Projectile(attacker.getCurrentTile(), defender.getCurrentTile(), defender, getDartProjectileGFX(weapon), 17, 30, 10, 31, 32)));
				}
			} else {
				...
				if (ammo != null && defender instanceof Player) {
					((Player) defender).getDispatcher().sendEvent(SceneUpdateEventEncoder.class, new SceneUpdateEventContext(new Projectile(attacker.getCurrentTile(), defender.getCurrentTile(), defender, getProjectileGFX(ammo), 17, 30, 10, 31, 32)));
				}
			}
...
//		defender.getCombat().queueBars(Bar.HITPOINTS);
	...
	/**
	 * 
	 * Checking and using Throwing Weapons
	 * @param attacker
	 * @param defender
	 */
	...
	private void sendArrowDrop(Entity attacker, Entity defender) {
		...
		if (region != null && region.isLoaded()) {
			...
			int oldAmt = 0;
			GroundItem oldItem = region.removeItem(defender.getCurrentTile(), ammo.getId());
			if (oldItem != null) {
