package org.virtue.model.entity.path.impl;
...
public abstract class AbstractPathfinder implements Pathfinder {
	 ...
	 * @return The path.
	 */
	public abstract Path find(Tile startCoords, int size, Tile end, int sizeX, int sizeY, int rotation, int type, int walkingFlag, boolean near);
	 ...
	 * @return The path.
	 */
	public static Path find(Tile start, Node destination) {
	 ...
	 * @return The path.
	 */
	public static Path find(Tile start, Node destination, boolean near, Pathfinder finder) {
	...
	public static Path find(Tile start, int moverSize, Node destination, boolean near, Pathfinder finder) {
		if (destination instanceof SceneLocation) {
			...
			return finder.find(start, moverSize, destination.getCurrentTile(), 0, 0, rotation, 1 + type, 0, near);
		}
		int size = 0;
	...
	public static Tile findAdjacent(Node node) {
		if (AbstractPathfinder.find((Entity) node, node.getCurrentTile().copyNew(-1, 0, 0), false, AbstractPathfinder.DUMB).isSuccessful()) {
			return node.getCurrentTile().copyNew(-1, 0, 0);
		}
		if (AbstractPathfinder.find((Entity) node, node.getCurrentTile().copyNew(0, -1, 0), false, AbstractPathfinder.DUMB).isSuccessful()) {
			return node.getCurrentTile().copyNew(0, -1, 0);
		}
		if (AbstractPathfinder.find((Entity) node, node.getCurrentTile().copyNew(1, 0, 0), false, AbstractPathfinder.DUMB).isSuccessful()) {
			return node.getCurrentTile().copyNew(1, 0, 0);
		}
