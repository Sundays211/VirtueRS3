...
module.exports = (function () {
	...
	//varp 4164 = queued ability action bar slot
	//varp 5861 = queued ability action bar ID (1003 = standard)
	...
	function runAbility (player, abilityId) {
		...
		//TODO: Check if an ability is currently active or the player isn't in combat. If so, queue the ability instead
		
		//TODO: Check if the player has enough adrenaline to perform the attack. If not, queue it instead
		
		//TODO: Drain or add adrenaline
		
		var target = player.getCombatSchedule().getAbilityLock();
		
		if (!target) {
			chat.sendDebugMessage(player, "Not in combat!");
			return;
		}
		
		switch (_config.structParam(abilityId, 2806)) {
		case 1://Attack
			attackAbilities.handle(player, target, abilityId);
			break;
		default:
			chat.sendDebugMessage(player, "Unhandled ability type: "+_config.structParam(abilityId, 2806));
			break;
		}
		...
		var abilityTime = Math.ceil(_config.seqLength(animation) / 30);
		
		util.runClientScript(player, 6570, [ 1, 1, cooldown, 0, abilityId ]);
		//util.runClientScript(player, 6066, [ abilityId ]);
		//TODO: Also store the cooldown on the server side & prevent the player performing the ability while on cooldown
		
		player.getCombatSchedule().cooldown(abilityTime);//Prevent further combat actions until the ability finishes
